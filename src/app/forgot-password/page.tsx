...
export default function ForgotPasswordPage() {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      await authApi.forgotPassword(forgotPasswordSchema.parse({ email }).email);
      setSent(true);
    } catch (err) {
      setError(
        err instanceof ApiError || err instanceof Error
          ? err.message
          : "Unable to send the OTP. Try again.",
      );
    } finally {
      setSubmitting(false);
    }
    ...
    <main className="min-h-screen flex items-center justify-center bg-canvas px-6 py-16">
      <div className="w-full max-w-sm">
        ...
        <p className="mt-1 mb-8 text-sm text-ink/60">
          We&apos;ll email a one-time password to your registered address.
        </p>
        {sent ? (
          <div className="space-y-5">
            <p className="rounded-lg border border-status-approved/30 bg-status-approved/10 px-3 py-3 text-sm text-ink">
              OTP sent. Check your email, then set a new password.
            </p>
