...
export async function PATCH(
  ...
  const canEdit =
    (isOwner && hasPermission(user, "blog.edit_own") && ["draft", "rejected"].includes(blog.status)) ||
    hasPermission(user, "blog.edit_any");

  if (!canEdit) {
    return NextResponse.json(
      { error: "This blog can't be edited in its current state, or you lack permission." },
      { status: 403 }
    );
  }
...
export async function DELETE(
  ...
  if (!hasPermission(user, "blog.delete")) {
    return NextResponse.json({ error: "You don't have permission to delete blogs." }, { status: 403 });
  }
