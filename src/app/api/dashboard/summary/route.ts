...
export async function GET(req: NextRequest) {
  ...
  const pendingReviewCount = scoped.filter((b) => b.status === "submitted_for_review").length;
  const pendingPublishCount = scoped.filter((b) => b.status === "approved").length;
  const recentlyPublished = scoped
    .filter((b) => b.status === "published")
    .sort((a, b) => (b.publishedAt ?? "").localeCompare(a.publishedAt ?? ""))
    .slice(0, 5);

  const recentActivity = auditLogs.filter((l) => l.companyId === companyId).slice(0, 8);
