...
export default function PublishQueuePage() {
  ...
  return (
    <AuthenticatedShell>
      <h1 className="mb-1 text-xl font-semibold text-ink">Publish queue</h1>
      <p className="mb-6 text-sm text-ink/60">
        Approved posts ready to go live.
      </p>
      <div className="divide-y divide-line overflow-hidden rounded-xl border border-line bg-panel">
        {loading ? (
          ...
          <div className="flex min-h-52 flex-col items-center justify-center px-4 py-8 text-center">
            <Send className="mb-3 h-7 w-7 text-ink/35" />
            <p className="text-sm font-medium text-ink">
              Nothing ready to publish
            </p>
            <p className="mt-1 text-sm text-ink/50">
              Approved posts will appear here when they are ready.
            </p>
          </div>
        ) : (
          blogs.map((blog) => (
            ...
            >
              <div className="min-w-0">
                ...
                <p className="mt-0.5 text-xs font-mono text-ink/40">
                  Approved {new Date(blog.updatedAt).toLocaleDateString()}
                </p>
              </div>
