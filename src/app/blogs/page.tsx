...
function BlogsContent() {
  const { activeCompanyId } = useAuth(); const router = useRouter(); const searchParams = useSearchParams(); const [search, setSearch] = useState(""); const [page, setPage] = useState(1); const [limit, setLimit] = useState(10);
  const status = (searchParams.get("status") ?? "") as BlogStatus | "";
  const { data, isLoading } = usePaginatedBlogs(activeCompanyId, status, page, limit);
  ...
  return <><div className="mb-6 flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-xl font-semibold text-ink">Blogs</h1><p className="mt-1 text-sm text-ink/70">Create, review, and publish your content.</p></div><RequirePermission permission="blog.create"><Link href="/blogs/new" className={buttonVariants({ variant: "primary" })}>Create Blog</Link></RequirePermission></div>
    ...
    {!isLoading && pagination && pagination.totalItems > 0 && <div className="mt-5 flex flex-wrap items-center justify-between gap-3"><div className="flex flex-wrap items-center gap-3"><p className="text-sm text-ink/70">Showing {((pagination.page - 1) * pagination.limit) + 1}–{Math.min(pagination.page * pagination.limit, pagination.totalItems)} of {pagination.totalItems} blogs</p><div className="w-24"><Input type="number" min={1} max={100} value={limit} onChange={(event) => { const nextLimit = Number(event.target.value); setLimit(Number.isFinite(nextLimit) && nextLimit > 0 ? Math.min(100, Math.floor(nextLimit)) : 1); setPage(1); }} placeholder="Rows" aria-label="Rows per page" /></div></div>{pagination.totalPages > 1 && <div className="flex items-center gap-2"><button type="button" onClick={() => setPage((current) => Math.max(1, current - 1))} disabled={pagination.page <= 1} className={buttonVariants({ variant: "secondary", size: "sm" })}>Previous</button><span className="px-2 text-sm text-ink/70">Page {pagination.page} of {pagination.totalPages}</span><button type="button" onClick={() => setPage((current) => Math.min(pagination.totalPages, current + 1))} disabled={pagination.page >= pagination.totalPages} className={buttonVariants({ variant: "secondary", size: "sm" })}>Next</button></div>}</div>}
  </>;
}
