...
type DashboardSummaryResponse = DashboardSummary | { status: number; message: string; data: DashboardSummary };

function normalizeDashboardSummary(response: DashboardSummaryResponse): DashboardSummary {
...
export function useDashboardSummary(companyId: string | null, canFilterByCompany = false) {
  return useQuery({
    ...
    queryFn: () => {
      ...
      return api.get<DashboardSummaryResponse>(`/api/v1/dashboard/summary${query ? `?${query}` : ""}`).then(normalizeDashboardSummary);
    },
