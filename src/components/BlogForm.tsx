...
export function BlogForm({
  ...
  function removeImage(index: number) {
    update(
      "coverImageUrl",
      values.coverImageUrl.filter((_, currentIndex) => currentIndex !== index),
    );
  }
        ...
        <section className="space-y-4 rounded-xl border border-line bg-panel p-5">
          ...
          >
            <button
              ...
              className={`border-b-2 px-3 py-2 text-sm font-medium ${contentMode === "write" ? "border-accent text-accent" : "border-transparent text-ink/55 hover:text-ink"}`}
            >
              Write
            </button>
            <button
              ...
              className={`border-b-2 px-3 py-2 text-sm font-medium ${contentMode === "preview" ? "border-accent text-accent" : "border-transparent text-ink/55 hover:text-ink"}`}
            >
              Preview
            </button>
          </div>
          {contentMode === "write" ? (
            ...
            <article className="article-content rounded-lg border border-line bg-canvas p-4 sm:p-6">
              {values.coverImageUrl[0] && (
                <img
                  src={values.coverImageUrl[0]}
                  alt=""
                  className="mb-6 h-52 w-full rounded-lg object-cover sm:h-72"
                />
              )}
          ...
          <section className="space-y-4 rounded-xl border border-line bg-panel p-5">
            ...
            >
              ...
              <button
                ...
                className={`${inputClassName} flex min-h-32 cursor-pointer flex-col items-center justify-center gap-2 border-dashed text-center disabled:cursor-not-allowed`}
              >
            ...
            {values.coverImageUrl.length > 0 && (
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                {values.coverImageUrl.map((imageUrl, index) => (
                  <div
                    key={`${imageUrl}-${index}`}
                    ...
                    {!readOnly && (
                      <div className="absolute right-2 top-2 flex gap-1">
                        ...
                        <button
                          type="button"
                          onClick={() => moveImage(index, 1)}
                          disabled={index === values.coverImageUrl.length - 1}
