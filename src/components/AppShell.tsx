...
export function AppShell({ children }: { children: React.ReactNode }) {
  const { user, hasPermission, logout, activeCompanyId, setActiveCompanyId } = useAuth();
    ...
    <div className="min-h-screen bg-canvas md:grid md:grid-cols-[280px_minmax(0,1fr)]">
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    active ? "bg-accent text-white font-medium" : "text-white/65 hover:bg-white/5 hover:text-white"
                  )}
                >
                  {item.label}
                </Link>
      ...
      <div className="flex min-w-0 flex-col">
        <header className="sticky top-0 z-20 flex h-14 items-center justify-between gap-3 border-b border-line bg-panel px-4 sm:px-6">
          <button aria-label="Open navigation" onClick={() => setDrawerOpen(true)} className="inline-flex h-9 w-9 items-center justify-center rounded-lg text-ink hover:bg-ink/5 focus-visible:ring-2 focus-visible:ring-accent/40 md:hidden"><Menu className="h-5 w-5" /></button>
          ...
          {user.companies.length > 1 ? (
            ...
            <span className="text-sm text-ink/60">{user.companies[0]?.name}</span>
          )}
          <ThemeToggle />
          <div className="relative">
            ...
            {profileOpen && (
              <div role="menu" aria-label="Profile menu" className="absolute right-0 z-30 mt-2 w-64 overflow-hidden rounded-xl border border-line bg-panel shadow-lg">
                <div className="border-b border-line px-4 py-3">
                  <p className="truncate text-sm font-semibold text-ink">{user.name}</p>
                  <p className="mt-0.5 truncate text-xs text-ink/70">{user.role?.label ?? "User"}</p>
                </div>
                <div className="p-1.5">
                  <Link href="/change-password" role="menuitem" onClick={() => setProfileOpen(false)} className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-ink/70 hover:bg-ink/5 hover:text-ink focus-visible:ring-2 focus-visible:ring-accent/40">
                    <KeyRound className="h-4 w-4" aria-hidden="true" />
                    Change password
                  </Link>
                  <button type="button" role="menuitem" onClick={() => logout()} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm text-ink/70 hover:bg-ink/5 hover:text-ink focus-visible:ring-2 focus-visible:ring-accent/40">
